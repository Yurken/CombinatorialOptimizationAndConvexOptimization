...
\textbf{问题}：用高斯-牛顿法拟合 $f(x, \theta) = \theta_1 e^{\theta_2 x}$，数据点 $(0, 2), (1, 3)$。初始 $\theta_0 = (1, 0)^T$。
...
\textbf{问题}：$\min\ f(x) = x_1 + x_2$，s.t. $x_1^2 + x_2^2 = 2$。验证 $x^* = (-1, -1)$ 是否满足一阶必要条件。
...
\textbf{问题}：$\min\ f(x) = 2x_1^2 + 2x_2^2 - 2x_1 x_2 - 4x_1 - 6x_2$
...
\textbf{问题}：$\min\ f(x) = x^2$，s.t. $x \geq 2$。用对数障碍函数，$x_0 = 3$，$\mu_0 = 1$。
...
\textbf{问题}：$\min\ \frac{1}{2}\|Ax - b\|^2 + \lambda\|z\|_1$ s.t. $x - z = 0$
