...
\begin{codebox}[单纯形法完整实现]
\begin{lstlisting}
import numpy as np

def simplex_method(c, A, b, maximize=True, max_iter=1000):
    """
    单纯形法求解标准型LP: max c^T x s.t. Ax = b, x >= 0
    ...
    for iteration in range(max_iter):
        ...
        cB = c_aug[basis]
        for j in nonbasis:
            T[0, j] = cB @ T[1:, j] - c_aug[j]
        T[0, -1] = cB @ T[1:, -1]
        
        history.append((basis.copy(), nonbasis.copy(),
                       T[0, -1]))
        
        # 最优性检验
        if all(T[0, j] >= -1e-10 for j in nonbasis):
            break
        
        # 选择入基变量
        entering = min(j for j in nonbasis
                      if T[0, j] < -1e-10)
        ...
        for i in range(m):
            if col[i] > 1e-10:
                ratios.append((T[i+1, -1] / col[i], i))
        leaving_idx = min(ratios)[1]
        leaving = basis[leaving_idx]
        
        # 枢轴运算
        pivot = T[leaving_idx + 1, entering]
        T[leaving_idx + 1] /= pivot
        for i in range(m + 1):
            if i != leaving_idx + 1:
                T[i] -= T[i, entering] * T[leaving_idx + 1]
        
        basis[leaving_idx] = entering
        nonbasis = [j for j in range(n + m) if j not in basis]
...
# 示例: max 3x1 + 5x2
# s.t. x1 + 2x2 <= 8, 3x1 + 2x2 <= 12, x1, x2 >= 0
c = np.array([3.0, 5.0])
A = np.array([[1.0, 2.0], [3.0, 2.0]])
b = np.array([8.0, 12.0])

x_opt, z_opt, hist = simplex_method(c, A, b)
print(f"最优解: x1 = {x_opt[0]:.4f}, x2 = {x_opt[1]:.4f}")
print(f"最优值: z = {z_opt:.4f}")
print(f"迭代次数: {len(hist)}")
for i, (basis, nonbasis, z_val) in enumerate(hist):
    print(f"  Iter {i}: basis={basis}, z={z_val:.4f}")
\end{lstlisting}
\end{codebox}
...
def bigM_method(c, A, b, constraint_types,
                maximize=True, M=1e6, max_iter=100):
    """
    大M法求解一般形式LP
    ...
    for i in range(m):
        ...
        new_cols = np.zeros((m, 0))
        
        if ct == '<=':
            s = np.zeros(m)
            s[i] = 1
            new_cols = s.reshape(-1, 1)
            c_aug = np.append(c_aug, 0)
            basis.append(n + slack_count)
            slack_count += 1
            var_types.append('s')
            ...
            a = np.zeros(m)
            a[i] = 1
            new_cols = np.column_stack([s, a])
            c_aug = np.append(c_aug, [0, -M])
            basis.append(n + slack_count + surplus_count
                        + artif_count + 1)
            surplus_count += 1
            artif_count += 1
            var_types.extend(['s', 'a'])
            
        elif ct == '=':
            a = np.zeros(m)
            a[i] = 1
            new_cols = a.reshape(-1, 1)
            c_aug = np.append(c_aug, -M)
            basis.append(n + slack_count + surplus_count
                        + artif_count)
            artif_count += 1
            var_types.append('a')
        ...
        if new_cols.size > 0:
            A_aug = np.hstack([A_aug, new_cols])
    
    total_vars = A_aug.shape[1]
    nonbasis = [j for j in range(total_vars) if j not in basis]
    
    # 构建单纯形表
    T = np.zeros((m + 1, total_vars + 1))
    T[1:, :total_vars] = A_aug
    T[1:, -1] = b
    
    # 消去人工变量在z行的系数
    for j in range(total_vars):
        T[0, j] = sum(c_aug[basis[i]] * T[i+1, j]
                     for i in range(m)) - c_aug[j]
    T[0, -1] = sum(c_aug[basis[i]] * T[i+1, -1]
                  for i in range(m))
    ...
    for _ in range(max_iter):
        ...
        ratios = [(T[i+1, -1] / col[i], i)
                 for i in range(m) if col[i] > 1e-10]
        leaving_idx = min(ratios)[1]
        ...
        basis[leaving_idx] = entering
        nonbasis = [j for j in range(total_vars)
                   if j not in basis]
    
    # 检查人工变量
    for bi in basis:
        if var_types[bi] == 'a' and abs(T[basis.index(bi)+1,-1]) > 1e-6:
    ...
    return x[:n], z, 'optimal'

# 示例
# max 3x1 + 2x2, s.t. 2x1+x2 >= 4, x1+2x2 >= 3, x1,x2 >= 0
c = np.array([3.0, 2.0])
A = np.array([[2.0, 1.0], [1.0, 2.0]])
b = np.array([4.0, 3.0])

x_opt, z_opt, status = bigM_method(c, A, b, ['>=', '>='])
print(f"状态: {status}")
print(f"最优解: x1={x_opt[0]:.4f}, x2={x_opt[1]:.4f}")
print(f"最优值: z={z_opt:.4f}")
\end{lstlisting}
...
def two_phase_method(c, A, b, constraint_types,
    ...
    for i in range(m):
        ...
        if ct == '<=':
            ...
            new_col[i] = 1
            A1 = np.hstack([A1, new_col])
            basis.append(var_count)
            var_count += 1
        elif ct == '>=':
            s_col = np.zeros((m, 1))
            s_col[i] = -1
            a_col = np.zeros((m, 1))
            a_col[i] = 1
            A1 = np.hstack([A1, s_col, a_col])
            basis.append(var_count + 1)
            artif_indices.append(var_count + 1)
            var_count += 2
        elif ct == '=':
            a_col = np.zeros((m, 1))
            a_col[i] = 1
            A1 = np.hstack([A1, a_col])
            basis.append(var_count)
            artif_indices.append(var_count)
            var_count += 1
    
    total1 = A1.shape[1]
    
    # Phase 1 objective: min sum of artificial vars
    c1 = np.zeros(total1)
    for ai in artif_indices:
        c1[ai] = 1
    
    T1 = np.zeros((m + 1, total1 + 1))
    T1[1:, :total1] = A1
    T1[1:, -1] = b
    
    # Eliminate artificial variables from obj row
    for j in range(total1):
        T1[0, j] = sum(T1[i+1, j] for i in range(m)
                      if (basis[i] in artif_indices)) - c1[j]
    T1[0, -1] = sum(T1[i+1, -1] for i in range(m)
                   if basis[i] in artif_indices)
    
    nonbasis1 = [j for j in range(total1) if j not in basis]
    
    # Phase 1 simplex (minimize, so reverse sign)
    for _ in range(max_iter):
        if all(T1[0, j] <= 1e-10 for j in nonbasis1):
            break
        entering = min((T1[0, j], j) for j in nonbasis1
                      if T1[0, j] > 1e-10)[1]
        col = T1[1:, entering]
        if all(col <= 1e-10):
            return None, None, 'unbounded'
        ratios = [(T1[i+1, -1] / col[i], i)
                 for i in range(m) if col[i] > 1e-10]
        leaving_idx = min(ratios)[1]
        pivot = T1[leaving_idx + 1, entering]
        T1[leaving_idx + 1] /= pivot
        for i in range(m + 1):
            if i != leaving_idx + 1:
                T1[i] -= T1[i, entering] * T1[leaving_idx + 1]
        basis[leaving_idx] = entering
        nonbasis1 = [j for j in range(total1) if j not in basis]
    ...
    keep = [j for j in range(total1) if j not in artif_indices]
    A2 = A1[:, keep]
    ...
    basis2 = []
    for bi in basis:
        if bi not in artif_indices:
            basis2.append(keep.index(bi) if bi in keep else None)
        else:
            # Replace with non-artificial variable
            for j in keep:
                col_j = A2[:, j]
                if abs(col_j[list(basis).index(bi)] - 1) < 1e-10:
                    basis2.append(keep.index(j))
                    break
    
    return _simplex_solve(c2, A2, b, basis2, maximize)

def _simplex_solve(c, A, b, basis_init, maximize):
    """内部单纯形求解"""
    m, n = A.shape
    basis = list(basis_init)
    T = np.zeros((m + 1, n + 1))
    T[1:, :n] = A
    T[1:, -1] = b
    ...
    for _ in range(100):
        ...
        if all(T[0, j] >= -1e-10 for j in nonbasis):
            break
        entering = min(j for j in nonbasis if T[0, j] < -1e-10)
        col = T[1:, entering]
        if all(col <= 1e-10):
            return None, np.inf, 'unbounded'
        ratios = [(T[i+1, -1] / col[i], i)
...
\begin{codebox}[对偶单纯形法]
\begin{lstlisting}
def dual_simplex(c, A, b, maximize=True, max_iter=100):
    """
    对偶单纯形法求解标准型LP: min c^T x s.t. Ax = b, x >= 0
    要求初始基对偶可行（检验数 >= 0）
    ...
    for _ in range(max_iter):
        ...
        for j in nonbasis:
            if row[j] < -1e-10:
                ratios.append((T[0, j] / abs(row[j]), j))
        entering = min(ratios)[1]
        
        # 枢轴运算
        pivot = T[r + 1, entering]
        T[r + 1] /= pivot
        for i in range(m + 1):
            if i != r + 1:
                T[i] -= T[i, entering] * T[r + 1]
        basis[r] = entering
...
\begin{codebox}[灵敏度分析]
\begin{lstlisting}
def sensitivity_analysis(c, A, b, cB_indices):
    """
    对当前最优基进行灵敏度分析
    ...
    for i in range(m):
        ...
        for j in range(m):
            if col[j] > 0:
                delta_max = min(delta_max, xB[j] / col[j])
            elif col[j] < 0:
                delta_min = max(delta_min, xB[j] / col[j])
        b_ranges.append((delta_min, delta_max))
    
    # 3. c_k的变化范围（基变量）
    c_ranges = []
    for idx in cB_indices:
        # 简化为 ±sigma/|系数|
        c_ranges.append((-1, 1))  # 需详细计算
...
result = sensitivity_analysis(c, A, b, basis)
print("影子价格:", result['shadow_prices'])
print("当前基解:", result['xB'])
\end{lstlisting}
\end{codebox}
