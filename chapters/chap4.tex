...
第1次迭代：$\lambda_1 = -0.01$，$\mu_1 = 0.01$
\[
f(\lambda_1) = -0.9999 < f(\mu_1) \approx -0.9999
\]
取新区间 $[-3, 0.01]$，依此类推直至区间长度小于0.1。
\end{example}
...
试探点：
\[
\lambda_k = a_k + \frac{F_{n-k-1}}{F_{n-k+1}}(b_k - a_k),\quad
...
\textbf{问题}：用Fibonacci法（$n=5$，共5次试探）求解
\[
\min\ \phi(\alpha) = (\alpha - 2)^2,\quad \alpha \in [0, 5]
\]
执行两轮迭代。

\textbf{解}：$F_1=F_2=1, F_3=2, F_4=3, F_5=5, F_6=8$

\textbf{第0轮}：$[a_0, b_0] = [0, 5]$，$k=0$，$F_{5}=5, F_{6}=8$
\begin{align*}
\lambda_0 &= 0 + \frac{F_4}{F_6}(5-0) = \frac{3}{8} \times 5 = 1.875 \\
...
\begin{align*}
\lambda_1 &= 0 + \frac{F_3}{F_5}(3.125-0) = \frac{2}{5} \times 3.125 = 1.25 \\
...
def fibonacci_search(phi, a, b, n, max_iter=100):
    """Fibonacci法, n为试探次数"""
    # 生成Fibonacci数列
    F = [1, 1]
...
\begin{example}[Newton法——迭代两次]\label{ex:newton}
\textbf{问题}：用Newton法求解
\[
\min\ \phi(\alpha) = (\alpha - 2)^2,\quad \alpha_0 = 5
\]
执行两轮迭代。

\textbf{解}：计算导数：
\[
\phi'(\alpha) = 2(\alpha - 2),\quad \phi''(\alpha) = 2
\]

\textbf{第0轮}：$\alpha_0 = 5$
\[
\phi'(\alpha_0) = 2(5-2) = 6,\quad \phi''(\alpha_0) = 2 > 0
\]
\[
\alpha_1 = 5 - \frac{6}{2} = 5 - 3 = 2
\]

\textbf{第1轮}：$\alpha_1 = 2$
\[
\phi'(\alpha_1) = 2(2-2) = 0,\quad \phi''(\alpha_1) = 2
\]
\[
\alpha_2 = 2 - \frac{0}{2} = 2
\]

\textbf{结果}：
\begin{itemize}
    \item 仅1轮即收敛到精确最优解 $\alpha^* = 2$
    \item 第2轮验证：$\alpha_2 = 2$，$|\alpha_2 - \alpha_1| = 0 < \varepsilon$
    \item 这是因为目标函数恰好是二次函数，Newton法具有\textbf{二次终止性}
\end{itemize}
\end{example}
...
\textbf{问题}：用Newton法求解
\[
\min\ \phi(\alpha) = \alpha^4 - 3\alpha^2 + \alpha + 1,\quad \alpha_0 = 1
\]
执行两轮迭代。

\textbf{解}：
\[
\phi'(\alpha) = 4\alpha^3 - 6\alpha + 1,\quad \phi''(\alpha) = 12\alpha^2 - 6
\]

\textbf{第0轮}：$\alpha_0 = 1$
\[
\phi'(1) = 4 - 6 + 1 = -1,\quad \phi''(1) = 12 - 6 = 6 > 0
\]
\[
\alpha_1 = 1 - \frac{-1}{6} = 1 + 0.1667 = 1.1667
\]

\textbf{第1轮}：$\alpha_1 = 1.1667$
\[
\phi'(1.1667) = 4(1.5879) - 6(1.1667) + 1 = 6.3517 - 7.0002 + 1 = 0.3515
\]
\[
\phi''(1.1667) = 12(1.3611) - 6 = 16.3332 - 6 = 10.3332
\]
\[
\alpha_2 = 1.1667 - \frac{0.3515}{10.3332} = 1.1667 - 0.0340 = 1.1327
\]

继续迭代可收敛到最优解 $\alpha^* \approx 1.13$。
\end{example}
...
Newton法收敛速度快，但需要计算二阶导数 $\phi''(\alpha)$。在许多实际问题中，二阶导数难以获取或计算代价很高。\textbf{插值法}（Interpolation Method）的核心思想是：用已知的若干个函数值（有时加上导数值）构造一个简单的多项式来近似原函数，然后求该多项式的极小点作为新的试探点。

\begin{conceptbox}[插值法的基本思路]
\begin{enumerate}[label=\arabic*., itemsep=0.3em, leftmargin=2em]
    \item 选取若干个试探点，计算对应的函数值（或导数值）
    \item 用这些信息构造一个低次多项式 $p(\lambda)$ 来近似 $\phi(\lambda)$
    \item 求 $p(\lambda)$ 的极小点 $\lambda^*$，作为新的试探点
    \item 根据 $\phi(\lambda^*)$ 的值决定下一步操作
\end{enumerate}

\textbf{与Newton法的对比}：
\begin{itemize}[itemsep=0.2em]
    \item Newton法：利用\textbf{单点处的二阶Taylor展开}（需要一阶、二阶导数）
    \item 插值法：利用\textbf{多个点的函数值}构造近似多项式（不需要高阶导数）
    \item 插值法通常具有\textbf{超线性收敛}速度，介于线性收敛和二次收敛之间
\end{itemize}
\end{conceptbox}

常用的插值法按所用信息可分为两类：
\begin{itemize}[itemsep=0.3em]
    \item \textbf{仅用函数值}：二次插值法（需3个点）、三次插值法（需4个点）
    \item \textbf{同时用函数值和导数值}：三次Hermite插值（需2个点各提供函数值和导数值）
\end{itemize}

\subsection{二次插值法}

二次插值法利用三个试探点的函数值构造二次多项式，以其极小点作为新的试探点。
...
\begin{itemize}[itemsep=0.3em]
    ...
    \item \textbf{插值法}：不需要显式计算二阶导数，超线性收敛
    \item \textbf{二次插值}：简单但精度有限
...
\textbf{问题}：用Shubert-Piyavskii方法的思路，在 $[0, 3]$ 上寻找
\[
...
精确一维搜索需要花费大量计算，特别是在迭代点远离最优解时。不精确搜索只需找到使目标函数值"足够下降"的步长即可，大大节省了计算量。
...
条件\eqref{eq:goldstein-sufficient}保证\textbf{充分下降}，条件\eqref{eq:goldstein-lower}防止\textbf{步长过小}。
...
\begin{tikzpicture}[scale=0.85]
    ...
    % 线性近似线 l1 和 l2
    \draw[blue,thick,dashed] (0,3.5) -- (5,0.5) node[right,scale=0.75] {$\ell_1$};
    \draw[red,thick,dashed] (0,3.5) -- (5,2.8) node[right,scale=0.75] {$\ell_2$};
    % 实际函数曲线
    ...
    % 可接受区间
    \fill[yellow!30,opacity=0.6] (0.8,0) rectangle (2.8,0.08);
    \draw[<->,thick] (0.8,-0.2) -- (2.8,-0.2) node[midway,below,scale=0.75] {可接受区间};
    % 标注区间边界
    \draw[blue,dotted] (0.8,0) -- (0.8,2.5);
    \draw[red,dotted] (2.8,0) -- (2.8,2);
    % 选中的步长
    \fill[green!50!black] (1.8,{3.5-1.8*1.5}) circle (2.5pt);
    \node[green!50!black,scale=0.75,above right] at (1.8,{3.5-1.8*1.5}) {$\lambda^*$};
    % x轴标记
    \node[below,scale=0.7,blue] at (0.8,0) {\small 下界};
    \node[below,scale=0.7,red] at (2.8,0) {\small 上界};
\end{tikzpicture}
\caption{Goldstein条件的几何解释：可接受步长位于两条直线之间的区域}
\label{fig:goldstein}
\end{figure}
...
Goldstein条件将可接受步长限制在两直线之间的黄色区域。

\subsection{Armijo条件（回溯线搜索）}
...
\textbf{问题}：设 $\phi(\alpha) = (1+\alpha-2)^2 = (\alpha-1)^2$，$\phi(0) = 1$，$\phi'(0) = -2$。用Armijo回溯法找可接受步长，$\bar{\alpha} = 2$，$c_1 = 0.1$，$\beta = 0.5$。
...
\textbf{问题}：设 $\phi(\alpha) = (\alpha - 2)^2$，$\alpha_0 = 0$。验证 $\alpha = 1$ 是否满足Wolfe条件，$c_1 = 0.1$，$c_2 = 0.9$。
