...
$\min\ f(x) = x^2$ s.t. $x \in [1, 3]$，$x_0 = 3$，$r = 1$。
...
用SA求解 $\min\ f(x) = x^2 - 4x + 4 = (x-2)^2$，$x \in [0, 5]$。
...
$\max\ f(x) = x^2$，$x \in [0, 31]$ 整数。群体大小 $N=4$，5位二进制编码。
